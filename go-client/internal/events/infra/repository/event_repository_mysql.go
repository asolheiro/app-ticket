...
// NewMysqlEventRepository creates a ne MySQL event repository.
...
func (r *mysqlEventRepository) CreateEvent(event *domain.Event) error {
	query := `
		...
		INTO
			events
			(id, name, location, organization, rating, rating, date, image_url, capacity, price, partnerID)
		VALUES
			(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
	_, err := r.db.Exec(
...
func (r *mysqlEventRepository) ListEvents() ([]domain.Event, error) {
	...
	for rows.Next() {
		var (
			eventID, eventName, eventLocation, eventOrganization, eventRating, eventImageURL sql.NullString
...
func (r *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	query := `
		SELECT
			id, name, location, organization, rating, date, image_url, capacity, price, partner_id
		FROM
			events
		WHERE
			id = ?
...
func (r *mysqlEventRepository) CreateSpot(spot *domain.Spot) error {
	...
	INTO 
		spots 
		(id, event_id, name, status, ticket_id)
	VALUES 
		(?, ?, ?, ?, ?)
	`
	_, err := r.db.Exec(
...
// FindSpotByName returns a spot for a given spotName and eventID
...
// CreatetTicket inserts a new ticket into the database
func (r *mysqlEventRepository) CreateTicket(ticket *domain.Ticket) error {
	query := `
		INSERT
		INTO
			tickets
			(id, event_id, spot_id, ticket_type, price)
		VALUES
			(?, ?, ?, ?, ?)
	`
