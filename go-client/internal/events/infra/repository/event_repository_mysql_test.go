...
func setupTestDB() error {
	...
	_, err = db.Exec(`
		CREATE TABLE spots (
			...
			FOREIGN_KEY (event_id) REFERENCES events(id)
...
func TestMysqlEventRepository(t *testing.T) {
	...
	t.Run("CreateEvent", func(t *testing.T) {
		...
		// Verify if the event was created
	...
	t.Run("CreatedSpot", func(t *testing.T) {
		err := setupTestDB()
		assert.Nil(t, err)
		...
		// Verify if the spot was created
	...
	t.Run("CreateTicket", func(t *testing.T) {
		...
		// Verify if the ticket was created
		storedSpot, err:= repo.FindSpotByID(spotID)
		assert.Nil(t, err)
